...
func setDefaults(v *viper.Viper) {
	...
	v.SetDefault("clients.pulp.client_cert", "")
	v.SetDefault("clients.pulp.client_key", "")
	v.SetDefault("clients.pulp.ca_cert", "")
	...
	v.SetDefault("clients.lightwell.coverage_uploads.url", "")
	v.SetDefault("clients.lightwell.coverage_uploads.name", "")
	v.SetDefault("clients.lightwell.coverage_uploads.access_key", "")
	v.SetDefault("clients.lightwell.coverage_uploads.secret_key", "")
	v.SetDefault("clients.lightwell.coverage_uploads.region", "")
	v.SetDefault("clients.lightwell.coverage_uploads.file_prefix", "")
	...
	if clowder.IsClowderEnabled() {
		...
		if clowder.LoadedConfig != nil {
			...
			if !ok {
				...
				v.Set("clients.lightwell.coverage_uploads.url", ClowderS3Url(*clowder.LoadedConfig.ObjectStore))
				v.Set("clients.lightwell.coverage_uploads.name", lightwellBucket.Name)
				log.Logger.Warn().Msgf("Bucket name: %v", lightwellBucket.Name)
				if lightwellBucket.Region == nil || *lightwellBucket.Region == "" {
					v.Set("clients.lightwell.coverage_uploads.region", "DummyRegion")
				} else {
					v.Set("clients.lightwell.coverage_uploads.region", lightwellBucket.Region)
				}
				if lightwellBucket.SecretKey == nil || *lightwellBucket.SecretKey == "" {
					log.Error().Msg("Object store secret Key is empty or nil!")
				} else {
					v.Set("clients.lightwell.coverage_uploads.secret_key", *lightwellBucket.SecretKey)
				}
				if lightwellBucket.AccessKey == nil || *lightwellBucket.AccessKey == "" {
					log.Error().Msg("Object store Access Key is empty or nil!")
				} else {
					v.Set("clients.lightwell.coverage_uploads.access_key", lightwellBucket.AccessKey)
				}
			}
		}
